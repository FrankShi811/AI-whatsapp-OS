# macOS and Windows v5.11.1 parity contract

This document is a release gate for the native Avalonia macOS build. The current
Windows WPF v5.11.1 implementation remains the product and behavior source of
...
| Navigation and motion | `MainWindow.xaml.cs` | 8 modules, Settings dialog, 110ms exit/235ms entry, 240ms expand/220ms collapse, delayed labels, v5.11.1 green focus rail without bottom dash | Static + UI smoke |
...
| Email Inbox | `EmailInboxView`, `EmailAccountWindow` | Provider guides and setup links, Keychain password, editable IMAP/SMTP TLS, test/save, background IMAP sync, compose/reply and CRM/AI context | Static + Core + UI smoke |
...
| Local-only boundary | `DataWorkspaceManager`, repositories, secret stores | Movable complete local workspace, SQLite, sessions/media and macOS Keychain; no shared customer cloud/PWA store | Static + migration/Core smoke |
| Packaging | Windows release + macOS workflow | Apple Silicon subtype-safe self-contained app, native Bridge, conditional Developer ID/notarization, DMG direct runtime plus Finder/LaunchServices UI smoke and SHA-256 | Runner gate |
...
suite, the native runtime smoke, the eight-module UI smoke, or the Finder /
LaunchServices launch gate fails.
