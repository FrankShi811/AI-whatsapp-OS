...
直接双击 `AI Sales OS.exe` 运行。当前版本为 1.10.0，主要数据继续保存在 `%LOCALAPPDATA%\WAFlow\waflow.db`，因此升级后仍会读取原有客户、账号、群发任务和设置。
...
- 首次启动显示五步蒙层引导，只把 DeepSeek 或兼容 AI 接口的 Base URL、API Key 和工作模型作为首次配置；系统通过 `/models` 自动拉取该 Key 可用的全部模型，用户直接选择，企业资料改为后续按需填写。右上角“使用引导”可随时重新打开。
