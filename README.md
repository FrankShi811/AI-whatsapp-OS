...
直接双击 `AI Sales OS.exe` 运行。当前版本为 1.16.1，主要数据继续保存在 `%LOCALAPPDATA%\WAFlow\waflow.db`，因此升级后仍会读取原有客户、账号、群发任务、分析报告和设置。
...
- 原生 Dashboard、商机智能、客户列表、WhatsApp Inbox、WhatsApp 自动化群发和客户智能分析中心。
