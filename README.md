...
直接双击 `AI Sales OS.exe` 运行。当前 Windows 版本为 5.13.1。默认本地数据工作区位于 `%LOCALAPPDATA%\WAFlow`；也可在“设置 → 本地数据工作区”安全迁移到 D、E 等其他本机固定磁盘。升级和迁移后仍会读取原有客户、账号、邮件历史、自动化任务、分析报告、知识库和设置。
