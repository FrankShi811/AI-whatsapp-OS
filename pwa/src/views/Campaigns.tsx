...
function CreateCampaign({ leads, onClose, onCreate }: { leads: ReturnType<typeof useStore>["leads"]; onClose: () => void; onCreate: (items: ReturnType<typeof useStore>["outreach"]) => Promise<void> }) {
  ...
  return <Modal title="创建人工触达任务" onClose={onClose} wide>
    <div className="form-grid">
      <Field label="渠道"><select value={channel} onChange={e => { setChannel(e.target.value as Channel); setSelected([]); }}><option value="whatsapp">WhatsApp</option><option value="email">邮件</option></select></Field>
      {channel === "email" && <Field label="邮件主题"><input value={subject} onChange={e => setSubject(e.target.value)}/></Field>}
      <Field label="消息正文"><textarea rows={5} value={body} onChange={e => setBody(e.target.value)} placeholder="可使用人工确认过的话术"/></Field>
    </div>
    <div className="audience-list"><div><strong>选择客户</strong><span>{selected.length} / {eligible.length}</span></div>{eligible.map(lead => <label key={lead.id}><input type="checkbox" checked={selected.includes(lead.id)} onChange={e => setSelected(ids => e.target.checked ? [...ids, lead.id] : ids.filter(x => x !== lead.id))}/><span>{lead.nickname || lead.name}</span><small>{channel === "whatsapp" ? lead.phone : lead.email}</small></label>)}</div>
