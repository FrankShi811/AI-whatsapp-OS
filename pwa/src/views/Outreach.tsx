  ...
  return <>
    ...
    <section className="inbox-layout">
      ...
      <Card className="conversation">
        <header><div><h2>{lead?.nickname || lead?.name}</h2><p>{lead?.company || "未填写公司"} · 本地人工记录</p></div>{channel === "whatsapp" ? <Bot/> : <Mail/>}</header>
        ...
        <div className="composer">
          {channel === "email" && <input value={subject} onChange={e => setSubject(e.target.value)} placeholder="邮件主题"/>}
          <textarea value={body} onChange={e => setBody(e.target.value)} placeholder="输入正文，或在右侧让 AI 生成可编辑草稿"/>
      ...
      <Card className="copilot">
        ...
        <Field label="这次想表达什么？"><textarea rows={5} value={intent} onChange={e => setIntent(e.target.value)} placeholder="例如：礼貌询问预计采购数量，并提供下一步产品资料。"/></Field>
        ...
        {risk && <div className="risk-note">{risk}</div>}
      </Card>
    </section>}
  </>;
}
