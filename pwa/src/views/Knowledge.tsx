...
export function Knowledge() {
  ...
  return <>
    ...
    <input hidden ref={fileRef} type="file" accept=".txt,.md,.csv,.json,.html,.htm,.xlsx,.xls" onChange={e => void upload(e.target.files?.[0])}/>
    ...
    <section className="knowledge-layout">
      <Card className="knowledge-list"><div className="section-title"><div><h2>已启用资料</h2><p>{knowledge.length} 份 · 当前浏览器</p></div></div>
        ...
        knowledge.map(doc => <div className="knowledge-row" key={doc.id}><span><FileText/></span><div><strong>{doc.name}</strong><small>{doc.text.length.toLocaleString()} 字符 · {new Date(doc.createdAt).toLocaleDateString("zh-CN")}</small></div><button onClick={() => void removeKnowledge(doc.id)}><Trash2 size={16}/></button></div>)}
      </Card>
      <Card className="retrieval-test"><h2>本地检索测试</h2><p>确认 AI 写作前能找到哪些已批准资料。</p>
        <div className="search"><Search/><input value={query} onChange={e => setQuery(e.target.value)} placeholder="输入产品、政策或客户问题"/></div>
